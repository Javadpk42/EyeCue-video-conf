...
const ProMainVideoPage = ()=>{
        ...
        const setAsyncOffer = async()=>{
            for(const s in streams){
                if(s !== "localStream"){
                    ...
                    console.log("Successfully set remote description for peer connection:", s);
    ...
    useEffect(()=>{
        ...
        const fetchDecodedToken = async()=>{
            const resp = await axios.post('http://localhost:3000/api/user/validate-link',{token},{ withCredentials: true });
    ...
    useEffect(()=>{
        ...
        const token = searchParams.get('token');
        const socket = socketConnection(token);
        proSocketListeners.proDashabordSocketListeners(socket,addIceCandidateToPc,dispatch);
    },[])
